...
//        data.put("start_created", df.format(calendar.getTime()));
                    data.put("start_created", "2021-06-01 01:03:13");
                    Date end = new Date();
//        data.put("end_created",  df.format(end));
                    data.put("end_created", "2021-06-02 23:59:13");
...
//        data.put("tid", "1702708671154565830");//bb
        data.put("tid", "1843545746397945832");
        data.put("sign", Utils.Sign(data,Config.AppSecret));
        // 调用服务API
    ...
    public void getTradeDetailBatch() throws Exception {
        ...
        data.put("timestamp", timestamp.toString());
//        data.put("tid", "1702708671154565830");//bb
        data.put("tids", "1702708671154565830,1645829892702565830,1024065443098565830");
        data.put("sign", Utils.Sign(data,Config.AppSecret));
        // 调用服务API
    ...
    public void getCaiNiaoWayBillCancel() throws Exception {
        ...
        Long timestamp = System.currentTimeMillis() / 1000;
        data.put("timestamp", timestamp.toString());
        data.put("cp_code", "STO"); //  物流公司code  比如，YUNDA
        data.put("waybill_code", "773096802678921"); // 电子面单号
        data.put("sign", Utils.Sign(data,Config.AppSecret));
        // 调用服务API
