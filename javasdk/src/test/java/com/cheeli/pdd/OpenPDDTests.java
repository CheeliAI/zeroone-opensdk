...
public class OpenPDDTests {
    ...
    public void logisticesCompanyOnineSend() throws Exception {
        ...
        // 订单号 ,必填
        data.put("order_sn", "x210722-655464919932723");
       //  快递公司编号，必填，  可通过查询快递公司列表获取，注意是Id不是code
        data.put("logistics_id","132"); //121 yunda
        // 快递单号,必填
        data.put("tracking_number","9883540049184");
...
//        data.put("end_updated_at", "1622086044" );
    ...
    public void getOrderListV2() throws Exception {
        ...
        data.put("refund_status", "5");
        // 注意日期格式
        data.put("start_confirm_at", "2021-10-29 00:00:00");
         // 注意日期格式
         data.put("end_confirm_at", "2021-10-29 19:34:00" );
        // 必填	返回页码 默认 1，页码从 1
        data.put("page", "1");
        // 必填	返回数量， 最大 30
        data.put("page_size", "30");
    ...
    public void getOrderDetail() throws Exception {
        ...
        data.put("timestamp", timestamp.toString());
        //  必填 ，订单号
        data.put("order_sn", "211108-367368682951606");
        // 参数签名
        data.put("sign", Utils.Sign(data,Config.AppSecret));
        List<BasicNameValuePair> params = new ArrayList<BasicNameValuePair>();
        for (Map.Entry<String, String> entry : data.entrySet()) {
...
//                "\t}";
    ...
    public void PddOrderBatchGet() throws Exception {
        ...
        data.put("appid",  Config.AppId);
        data.put("seller_nick", seller_nick);
        data.put("tids", "210629-425092772742522");
        Long timestamp = System.currentTimeMillis() / 1000;
        data.put("timestamp", timestamp.toString());
        // 参数签名
        data.put("sign", Utils.Sign(data,Config.AppSecret));
    ...
    public void PddWayBillCancel() throws Exception {
        ...
        //   非必填	 电子面单号
        data.put("waybill_code",  "9866025654820");
        //  非必填	 快递公司code
...
//                "]";
        ...
        String requestData = "[ {\"data_tag\":\"211031-295693218792299x\",\"encrypted_data\":\"~AgAAAAECEAAFIunjGQCo3KpIwTl2jOm6LLcCAfZIh1A=~Lvugo3Pv~0~~\"}]";
